...
class Executor:
    ...
    def initialize(self) -> bool:
        try:
            ...
            # Drop our reference to the raw key. ClobClient still holds what it
            # needs internally; we don't want a plaintext copy hanging around
            # on this object for the rest of the process lifetime.
    ...
    def buy(self, token_id: str, amount_usd: float, price: float = 0.0) -> OrderResult:
        ...
        has a fresh price, saves one CLOB roundtrip at execution time).
        """
        ...
        if self.dry_run:
            ...
            if shares < 1:
                return OrderResult(
                    success=False, status=REJECTED,
                    error=f"Can't afford 1 share at ${sim_price:.3f} within ${amount_usd:.2f}",
                    side="BUY", price=sim_price,
                )
            return OrderResult(
        ...
        # Apply Polymarket minimum notional as floor (exchange requirement, not strategy)
        if clean_amount < POLY_MIN_NOTIONAL:
            effective_amount = POLY_MIN_NOTIONAL
            shares, clean_amount = calculate_order_size(market_price, effective_amount)
            if shares >= 1:
                print(f"  📐 Kelly ${amount_usd:.2f} → bumped to exchange min ${POLY_MIN_NOTIONAL:.0f}")
            else:
                return OrderResult(
                    success=False, status=REJECTED,
                    error=f"Can't afford 1 share at ${market_price:.3f} even at ${POLY_MIN_NOTIONAL:.0f} minimum",
                    side="BUY", price=market_price, token_id=token_id[:16] + "...",
                )
