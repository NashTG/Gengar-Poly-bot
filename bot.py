...
from market import get_current_market, fetch_clob_best_ask, current_window_ts, PERIOD_SECONDS
...
class PolyBot:
    def __init__(self):
        ...
        _pk = get_secret("PRIVATE_KEY")
        self.executor = Executor(
            private_key=_pk,
            safe_address=os.getenv("SAFE_ADDRESS", ""),
            dry_run=self.dry_run,
        )
        # Drop the local reference so the key isn't reachable from this scope.
        # Executor.initialize() also clears its own copy after ClobClient setup.
        del _pk
    ...
    def _get_market_prices(self, btc_price: float, seconds_remaining: float) -> tuple:
        ...
        try:
            ...
            if self.dry_run or not self.executor._initialized:
                # Public CLOB order book — no auth required, real-time prices
                up_price = fetch_clob_best_ask(market.token_id_up)
                down_price = fetch_clob_best_ask(market.token_id_down)
                # Fall back to Gamma outcomePrices if book is empty
                if up_price <= 0:
            ...
            # Live: probe complement engine for real fill prices
            probe_amount = 5.0
    ...
    def _execute_trade(self, sig, seconds_remaining: float):
        ...
        # so executor skips a second fetch (saves one CLOB roundtrip ~500ms).
        # In dry_run, pass the Gamma-derived price so simulation is realistic.
