...
def fetch_clob_best_ask(token_id: str) -> float:
    """Get best ask price from the public CLOB order book (no auth required).

    For a BUY order, the fill price is the lowest ask in the book.
    ...
    try:
        ...
        return round(best, 4) if 0 < best < 1 else 0.0
    except Exception:
        return 0.0
